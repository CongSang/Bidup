    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.length > 0 ? $.each(data, function(index, notif){
                var li = `  ${notif.type === 'JOIN_AUCTION' ?
                                    `<li  class="dropdown-item d-flex align-items-center notif-loading w-100 ${notif.is_read && `is-read-notify`}">
                                    <div class="notif-item" onclick="toAuction(${notif.postId}, '${notif.notifId}', '${notif.type}')">
                                        ...
                                        <div class="notif-item--message">
                                            <span class="mb-1 message">  
                                                ...
                                                `<strong>${notif.last_modified_name}</strong>`} đã tham gia đấu giá bài viết của bạn
                                            </span>
                                ...
                                : `<li  class="dropdown-item d-flex align-items-center notif-loading w-100 ${notif.is_read && `is-read-notify`}">
                                    <div class="notif-item" onclick="toPost(${notif.postId}, '${notif.notifId}', '${notif.type}')">
                                        ...
                                        <div class="notif-item--message">
                                            <span class="mb-1 message">  
                                                ${notif.count > 1 ? `<strong>${notif.last_modified_name}</strong> và ${notif.count - 1} người khác`: 
                                                `<strong>${notif.last_modified_name}</strong>`} đã ${notif.type === 'REACT_POST' ? `thích`:`bình luận về`} bài viết của bạn
                                            </span>
                                            <span class="notif-time ${notif.is_read && `is-read-notify`}">${moment(notif.last_modified).fromNow()}</span>
                                        </div>
                                        <div class="notif-dot" ${notif.is_read && `style="display:none;"`}>
